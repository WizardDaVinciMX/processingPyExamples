...
def customize(DropdownList ddl) :
...
def keyPressed() :
  ...
  if (key=='1') :
    ...
    # add items to the pulldown menu
    n = (int)(random(100000))
    d1.addItem("item "+n, n)
...
def controlEvent(ControlEvent theEvent) :
  ...
  # if (theEvent.isGroup())
  # to adef an error message thrown by controlP5.
...
controlP5.DropdownList : getBackgroundColor() 
controlP5.DropdownList : getBarHeight() 
controlP5.DropdownList : getHeight() 
controlP5.DropdownList : def controlEvent(ControlEvent) 
controlP5.DropdownList : def keyEvent(KeyEvent) 
controlP5.DropdownList : def setDirection(int) 
controlP5.DropdownList : def updateItemIndexOffset() 
