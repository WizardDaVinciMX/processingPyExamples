...
def setup()
{
  ...
  envFollow = EnvelopeFollower( 0,   # attack time in seconds
...
def draw()
{
  ...
  amp = constrain( map( mouseY, 0, height, 1, 0), 0, 1 )
  ...
  freq = constrain( map( mouseX, 0, width, 0.1, 1 ), 0.1, 1 )
  ...
  h = envFollow.getLastValues()[0] * height
