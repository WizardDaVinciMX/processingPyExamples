"""*
  ...
  same methods that AudioPlayer does for controlling the file playback: play(), loop(), cue(position), etc.
  <p>
...
add_library("ddf.minim.spi.* # for AudioRecordingStream
...
def mousePressed()
{
  pos = map(mouseX, 0, width, 0, filePlayer.length())
...
def draw()
{
 ...
 :
  ...
  songPos = map( filePlayer.position(), 0, filePlayer.length(), 0, width )
