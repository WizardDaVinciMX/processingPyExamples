...
class PanInstrument implements Instrument
{
  ...
  PanInstrument( oscFrequency, oscAmplitude, lfoFrequency, lfoAmplitude )
 :    
    # create instances of any UGen objects as necessary
    sineOsc = Oscil( oscFrequency, oscAmplitude, Waves.SINE )
    ...
    LFO = Oscil( lfoFrequency, lfoAmplitude, Waves.SINE )
...
def setup()
{
  ...
  PanInstrument myNote = PanInstrument( 587.3f, 0.5, 0.5, 1.0 )
  
  # play a note with the myNote object
  out.playNote( 0.5, 2.6, myNote )
  
  # give a note value to myNote
  myNote = PanInstrument( 415.3f, 0.5, 3.0, 0.5 )
