...
class BumpyInstrument implements Instrument
{
  ...
  BumpyInstrument( String pitch, amplitude )
 :    
    # calculate the frequency for the oscillator from the note name    
    frequency = Frequency.ofPitch( pitch ).asHz()
    ...
    Wavetable myEnv = WavetableGenerator.gen7( 8192, 
        float[]: 0.00, 1.00, 0.15, 1.00, 0.00 , 
        int[]  : 1024, 1024,   64, 6080  )

    # create instances of any UGen objects as necessary
    # The tone is the first five harmonics of a square wave.
    toneOsc = Oscil( frequency, 1.0f, Waves.squareh( 5 ) )
    envOsc = Oscil( 1.0f, amplitude, myEnv )
...
def setup()
{
  ...
  out.playNote( 0.5, 2.6, BumpyInstrument( "A4", 0.5 ) )
  out.playNote( 2.5, 1.6, BumpyInstrument( "F4", 0.5 ) )
  out.playNote( 3.6, 0.9, BumpyInstrument( "D4", 0.5 ) )
