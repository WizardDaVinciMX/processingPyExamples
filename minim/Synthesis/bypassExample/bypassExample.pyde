...
def setup()
{
  ...
  Delay myDelay = Delay( 0.6, 0.9, true, true )
  
  # create a Bypass to wrap the Delay so we can turn it on and off
  bypassedDelay = Bypass<Delay>( myDelay )
  
  # create the Blip that will be used
  Oscil myBlip = Oscil( 245.0, 0.3, Waves.saw( 15 ) )
  
  # create an LFO to be used for an amplitude envelope
  Oscil myLFO = Oscil( 1, 0.3, Waves.square( 0.95 ) )
