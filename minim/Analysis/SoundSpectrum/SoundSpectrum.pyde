...
def draw()
{
 ...
 :
    ...
    for(i = 0 i < fftLin.avgSize() i+=1)
   :
      # if the mouse is inside the bounds of this average,
      # prthe center frequency and fill in the rectangle with red
 ...
 :
    ...
    for(i = 0 i < fftLog.avgSize() i+=1)
   :
      centerFrequency    = fftLog.getAverageCenterFrequency(i)
      # how wide is this average in Hz?
      averageWidth = fftLog.getAverageBandWidth(i)   
      
      # we calculate the lowest and highest frequencies
      # contained in this average using the center frequency
      # and bandwidth of this average.
      lowFreq  = centerFrequency - averageWidth/2
      highFreq = centerFrequency + averageWidth/2
      
      # freqToIndex converts a frequency in Hz to a spectrum band index
      # that can be passed to getBand. in this case, we simply use the 
      # index as coordinates for the rectangle we draw to represent
      # the average.
      xl = (int)fftLog.freqToIndex(lowFreq)
      xr = (int)fftLog.freqToIndex(highFreq)
      
      # if the mouse is inside of this average's rectangle
      # prthe center frequency and set the fill color to red
