...
from waveform import WaveformRenderer
...
def setup():
  ...
  saw = SawWave(100, 0.2, out.sampleRate())
...
def draw():
  ...
  if out.hasControl(Controller.PAN):
    text("The current pan value is " + str(out.getPan()) + ".", 5, 15)
  else:
  ...
  if out.hasControl(Controller.BALANCE):
    text("The current balance value is " + str(out.getBalance()) + ".", 5, 45)
  else:
    text("The output doesn't have a balance control.", 5, 45)
  
  if out.hasControl(Controller.GAIN):
    text("The current gain value is " + str(out.getGain()) + ".", 5, 60)
  else:
    text("The output doesn't have a gain control.", 5, 60)

def keyReleased():
  if key == 'v':
    out.shiftVolume(0, 1, 2000)
  elif key == 'g':
    out.shiftGain(-40, 0, 2000)
  elif key == 'b':
    out.shiftBalance(-1, 1, 2000)
  elif key == 'p':
    out.shiftPan(1, -1, 2000)
